...
- Windows installer는 `installer/CdxSwap.Setup` WinUI 3 bootstrapper로 추가됐다. Tauri MSI를 embedded payload로 포함하고, silent MSI 실행, 로그 경로, 실패 exit code, 설치 후 Launch 흐름을 제공한다.
...
- WinUI 3 installer의 Linux 로컬 빌드는 이 환경에 .NET SDK가 없어 실행하지 못했고, Windows GitHub Actions에서 최종 확인해야 한다.
