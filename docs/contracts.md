...
`rename_profile` 입력은 `profileId`와 `nextProfileId`다. 백엔드는 빈 값, 중복 이름, 경로 탐색, Windows 예약 문자를 거부한다. 성공 시 modern profile 디렉터리를 이동하고 새 `ProfileRecord`를 반환한다. rename은 auth 파일을 복사하거나 재작성하지 않고 프로필 이름과 home 경로만 바꾼다.
