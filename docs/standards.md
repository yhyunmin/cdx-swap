...
React 패널 스타일은 4px spacing scale을 기준으로 한다. 기본 텍스트 색은 `#333`, 보조 텍스트와 placeholder는 `#888`, 활성 사용량 행 배경은 `#fff0f3`이다. 폰트 크기는 12px, 14px, 16px, 18px 단위만 쓰고, weight는 300, 400, 500 안에서 유지한다. 패널 shell radius는 24px, 내부 컨트롤은 4px 또는 8px를 사용한다.

아이콘 전용 액션은 `aria-label`과 styled tooltip을 함께 가져야 한다. tooltip은 200ms 지연, 흰 배경, 부드러운 그림자, `#333` 텍스트를 기준으로 한다. icon action을 행 클릭 영역 안에 중첩하지 않는다.
...
Windows setup UI는 legacy wizard 화면을 노출하지 않는다. Ready, Installing, Success, Failure 상태를 한 창 안에서 처리하고, 실패는 항상 MSI exit code와 log path를 보여준다. installer runtime은 offline 실행을 위해 unpackaged self-contained WinUI 3 single-file publish 설정을 유지한다.
