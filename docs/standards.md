...
Windows setup UI는 legacy wizard 화면을 노출하지 않는다. Ready, Installing, Success, Failure 상태를 한 창 안에서 처리하고, 실패는 항상 MSI exit code와 log path를 보여준다. installer는 setup 크기를 줄이기 위해 Windows 기본 .NET Framework 기반의 lightweight WPF bootstrapper로 유지한다.
