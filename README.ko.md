...
cross-build 결과는 개발 확인용이며 공식 배포물은 GitHub Release의 NSIS installer와 portable zip입니다.
...
- Profile별 Login / Run / Logout을 Codex CLI로 실행합니다.
...
앱은 profile별로 `CODEX_HOME`을 다르게 지정해서 Codex CLI를 실행합니다.

- `Login`: 해당 profile의 Codex login flow를 시작합니다.
- `Run`: 해당 profile로 Codex CLI를 실행합니다.
