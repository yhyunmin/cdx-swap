...
cd "$ROOT"
npm run build
PATH="$ROOT/scripts/cross-tools:$PATH" DEP_TAURI_DEV=false RUSTFLAGS="${RUSTFLAGS:-} -C target-feature=+crt-static" cargo xwin build --manifest-path src-tauri/Cargo.toml --target x86_64-pc-windows-msvc --release
